...
import static com.cleanup.todoc.utilities.TestUtils.withRecyclerView;
 ...
 * @modified by student Tommy MARCHAL
...
public class MainActivityInstrumentedTest {
    ...
    public void init() {
        ...
        taskRepository = Injection.provideTaskRep(this.context);
        projectRepository = Injection.provideProjectRep(this.context);
        viewModel = new TodocViewModel(taskRepository, projectRepository);
        viewModel.setFilter(utils.SortMethod.NONE);
        // MAKING SURE THE LIST IS EMPTY BEFORE EACH TEST
    ...
    @Test
    public void keepTasksOrderWhileRebuildingView() {
        // Creating 3 tasks :
        onView(withId(R.id.fab_add_task)).perform(click());
        onView(withId(R.id.txt_task_name)).perform(replaceText("aaa Tâche example"));
        onView(withId(android.R.id.button1)).perform(click());
        ...
        // Checking if they're in the same order
        onView(withRecyclerView(R.id.list_tasks).atPositionOnView(0, R.id.lbl_task_name))
                .check(matches(withText("zzz Tâche example")));
        onView(withRecyclerView(R.id.list_tasks).atPositionOnView(1, R.id.lbl_task_name))
                .check(matches(withText("hhh Tâche example")));
